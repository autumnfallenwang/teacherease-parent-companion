...
**Status:** in-progress — picker live, dates fixed. Settings → Appearance → Language with 4 buttons (System / English / Español / 中文). All four `toLocaleDateString(undefined, ...)` callers swapped to take explicit `Locale`. Smoke-tested 2026-05-05: three switches fired clean, sidebar swapped, dates re-formatted, no errors. The original "Chinese dates with English UI" symptom is resolved. Translated surfaces: sidebar tabs + settings sub-tabs + brand + back/expand/collapse + every date in the app. Remaining extraction (~21 components' worth of body text, error toasts, wizard, About) deferred — mechanical and resumable, but not blocking the user-visible win. Branch `feat/i18n-language-picker` ready for v0.1.8.
