...
**Status:** in-progress — picker + dates + B1 + B2 done. **B2 (Settings panels + Wizard + About + Gmail guide + sidebar child selector) shipped 2026-05-05**: ~190 keys × 3 locales covering Children / Appearance / Attention / Fetch / Notifications / Email / Advanced settings panels (validation messages, status states, button labels, help text, descriptions), wizard disclaimer flow (legal text moved into `wizard.disclaimer.*` catalog keys; English constants stay in `legal.ts` for non-runtime callers), About page sections, "Viewing" eyebrow on the child selector, full Gmail App Password walkthrough (5 step headings + bodies + 4 troubleshooting bullets + intro). Smoke-tested across en/es/zh — all translated end-to-end, no key fall-through. **Total catalog now ~245 keys × 3 locales.** Remaining: B3 (Email digest body + OS notification body + scraper error toasts + `Intl.RelativeTimeFormat` for "in 2h" / "5m ago" / "due now" — ~30 keys). Branch `feat/i18n-language-picker` ready for v0.1.8.
