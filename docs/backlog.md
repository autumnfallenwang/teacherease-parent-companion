...
**Status:** in-progress — picker + dates + B1 done. Settings → Appearance → Language with 4 buttons (System / English / Español / 中文). All four `toLocaleDateString(undefined, ...)` callers swapped to explicit `Locale`. **B1 (Today / Classes / History page bodies) shipped 2026-05-05**: 39 keys × 3 locales covering page headers, section headings, status pills (Needs Attention / Meeting / Not Assessed → translated), standards tree text, attention section, recent activity templates with class-name interpolation, homework section headings + empty states, history empty states + Month label, the Welcome / Add-first-child empty state. Plurals handled with `.one`/`.other` keys per Q37. Portal-fetched content stays verbatim per Q37 amendment. Smoke-tested across all 3 locales × 3 pages, no errors. Remaining: B2 (Settings panels + Wizard + About + child switcher — ~150 strings) and B3 (Email digest + OS notification + scraper errors + `Intl.RelativeTimeFormat` for "in 2h" / "5m ago" — ~30 strings). Branch `feat/i18n-language-picker` ready for v0.1.8 if you want to ship now or wait for B2/B3.
