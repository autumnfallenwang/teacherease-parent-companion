...
**Status:** done — shipped via Phase 29. `:root` + `.dark` adopt shadcn Stone (verbatim 2025 OKLCH). `.theme-solarized{,.dark}` adopts canonical Schoonover hex. `.theme-nord{,.dark}` adopts canonical arcticicestudio hex (Snow Storm light, Polar Night dark). `.theme-dracula{,.dark}` adopts canonical Rocha hex (community lavender derivation for light). `.theme-contrast{,.dark}` keeps the custom WCAG-AAA target with minor tightening. Token surface (names, `@theme inline`, settings keys) unchanged — only values updated. B-01 / B-02 / B-05 (the composition bugs that motivated this swap) were already closed in earlier component-side work; verified not regressed under the new tokens.
