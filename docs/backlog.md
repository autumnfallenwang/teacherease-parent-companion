...
**Status:** in-progress — foundation laid on `feat/i18n-language-picker`. Per-file extraction is mechanical and resumable. Phase 32 in `docs/progress.md` has the partial-state task table; "current state" paragraph at the bottom names the highest-priority follow-up (swap `toLocaleDateString(undefined, ...)` to use `formatDate()` so dates and UI agree — this is the actual fix for the original Chinese-dates / English-UI symptom). Picker UI + remaining ~21 components deferred to a future release.
