...
## B-22 — Rust scheduler uses monotonic countdown, silently skips fires across system sleep
**Where:** `src-tauri/src/scheduler.rs:112-115` — `spawn_worker` uses `tokio::time::sleep_until(Instant::now() + delta)`. `Instant` is monotonic.
**Observed:** User set notify slot for 15:15 EDT on 2026-05-08. App was running the whole time (log confirms — no app restart, only Dashboard re-mounts at 15:17 / 15:18). Earlier 09:00 EDT fetch fired fine across clamshell sleep, but the next 15:00 fetch and 15:15 notify never fired — log shows complete silence between `fetch armed for fire_at_ms=1778266800000` at 09:00:04 and `frontend logging initialized` at 15:17:55. Same pattern explains the previous day's "22-minute fetch" (`durationMs=1361154` on May 7 21:00 EDT) and 14-minute fetch (`durationMs=838101` on May 8 01:00 UTC) — the renderer fetch was paused mid-flight, not slow. Root cause: monotonic clock pauses with the process during macOS sleep, so the 6-hour countdown can resume with hours still "remaining" hours after the wall-clock target has passed. Fires happen at random delayed times or not at all. This is the same bug class as the original B-20 (webview throttling) but one layer down — moving the timer into Rust didn't help because `Instant`-based sleep has the same defect as `setTimeout` does inside a throttled webview.
**Proposed:** Convert the single `sleep_until(Instant)` deadline to a wall-clock polling loop. Library research (tokio-cron-scheduler / clokwerk / job_scheduler_ng / delay_timer / cron) — closest fit `tokio-cron-scheduler` is wall-clock-based but pulls a job-storage abstraction we don't need and would force adopting cron strings as the schedule format, rewriting the TS↔Rust contract; `clokwerk` is unmaintained since May 2023; `job_scheduler_ng` still requires user-driven `tick()` (same code we'd write); none preserve our `tokio::sync::Notify` cancel ergonomics. Hand-roll. Replace lines 110-127 with: `loop { let now_ms = chrono::Utc::now().timestamp_millis(); if now_ms >= target_ms { break; }; let nap = ((target_ms - now_ms).min(30_000) as u64).max(1); tokio::select! { _ = tokio::time::sleep(Duration::from_millis(nap)) => {} _ = slot.cancel.notified() => { return /* outer loop re-reads */ } } }` — then on break, check `now_ms - target_ms` against a 5-minute miss threshold: if ≤ 5 min late, fire (catch the ±60s wake window); if > 5 min late, log `scheduler-rust: {kind} missed (woke {N}min late) — skipping to next slot`, clear `fire_at_ms`, and let the webview re-arm next slot when its event listener fires. Cron-style "miss = skip" semantics matching user expectation. The 30-second cap on each `sleep` ensures even if `tokio::time::sleep` itself gets paused on suspend (it's also `Instant`-based internally), we re-enter the loop on wake, call `Utc::now()`, and make a fresh decision. No new dependencies (chrono is already in Cargo.toml). Existing `Notify` cancel/re-arm contract unchanged. Schema unchanged. TS unchanged. Add unit tests: arm a target 2 sec out, advance fake clock, assert fire on-time; arm a target, advance fake clock past `target + 10 min`, assert NOT fired and slot cleared. Manual smoke test: arm a target 90 sec out, close lid for 30 sec, open → should still fire within ~60s of target. Effort ~1-2 hours (~20 lines changed in `scheduler.rs` + ~40 lines of unit tests).
**Status:** open
...
## D-24 — Localized UI: language picker (English / Spanish / Chinese), follow OS by default
**Where:** Cross-cutting. New `src/lib/i18n.ts` (~50 lines, hand-rolled `t()`), three flat-JSON catalogs at `src/lib/locales/{en,es,zh}.json`, every component file with user-visible English literals (~39 components), Settings → Appearance gets a "Language" picker (sibling of theme / font size).
**Observed:** A user (the maintainer's wife) opened the app on a Chinese-locale Windows desktop. **All static UI strings rendered in English** — "Today", "Attention", "Missing assignment", labels, button text, etc. **But homework date headers and "last fetch" timestamps rendered in Chinese.** The mismatch traces to `toLocaleDateString(undefined, ...)` calls in `homework-card.tsx`, `history-view.tsx`, `settings-fetch.tsx`, `settings-notifications.tsx` — passing `undefined` as locale tells JS to follow the OS, which silently localized those strings. Static text has no such pathway, so it stayed English. Net: the UI looks half-translated, which is worse than fully-English. Three real audiences are visible: (a) Chinese-language parents (tested locale), (b) Spanish-language parents (geographic likelihood given many TeacherEase districts), (c) English defaults for everyone else.
**Proposed:** Hand-rolled i18n (no library — one `t(key)` helper + three JSON catalogs ≈ ~25 KB total bundle, vs ~30 KB+ for `i18next` + `react-i18next` for an app this size). New Settings → Appearance row: **Language: System (default) / English / Español / 中文**. "System" reads `navigator.language` and matches the most-specific catalog (`zh-CN` → `zh`, `es-MX` → `es`, anything else → `en`). User override persisted to `settings` key `ui.language` with values `system | en | es | zh`. The `t()` helper does `catalog[key] ?? en[key] ?? key` so missing translations fall through to English (and a missing-key bug renders the key string, easy to spot in dev). Date locale follows the same setting — pass an explicit locale to `toLocaleDateString` so dates and UI agree (today's mismatch goes away). Initial Spanish + Chinese catalogs cover **high-traffic strings only** (Today / Classes / History / Settings tab labels, "Missing", "Low score", "Add child", common error toasts); long-tail strings stay English-only and translate later if real users surface gaps. Effort ~1 day for the scaffolding + extraction; translation quality pass is separate and incremental. Bundle impact ~25 KB on a 1.5 MB JS payload (~1.5%). New Q37 locks the architecture (hand-rolled, OS-default, English fallback, settings-table-driven).
**Status:** done — Phase 32 complete (B1 + B2 + B3 shipped). **Total catalog: ~295 keys × 3 locales** covering every app-generated surface — sidebar tabs, all settings panels, wizard disclaimer flow, About page, Gmail App Password walkthrough, child selector, Today / Classes / History page bodies (status pills, attention rows, recent-activity templates, homework section headings + dates, history empty states), email digest (subject + section headings + footer), OS notification (title + body), scraper error toasts (login + homework-URL via error codes + catch-site translation), relative-time chips ("en 5m" / "5 分钟后"). Architecture: hand-rolled `t()` + `formatDate` / `formatDateTime` / `formatPortalDate` / `formatRelative` helpers + locale-as-parameter through non-React notify pipeline. Language picker = `[System toggle]` + dropdown in one bordered card (scales to 4+ languages by adding a `LANGUAGES` row + JSON file). Portal-fetched content (class names, assignment names, teacher names, grade values, homework subject/content) stays verbatim per Q37 amendment. Smoke-tested across en/es/zh end-to-end. Branch `feat/i18n-language-picker` ready for v0.1.8.
