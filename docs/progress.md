...
**Phase 9 complete.** `handleRefresh` is now a thin `buildFetchRunner(...).runAll(child)` call; every user-visible notification flows through `NotifyRouter`; observability unified in `fetch_runs`; silent scrape failures now surface via opt-in `fetchFailed` notification. Dashboard UI visually unchanged. Next: **Phase 10** (Desktop shell — sidebar-shell app; start with **S1** restructuring routes into Today / Classes / History / Settings / About).
