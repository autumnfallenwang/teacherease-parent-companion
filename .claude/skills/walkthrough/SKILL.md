...
3. Skim `docs/design-plan.md`'s "Locked Decisions" section (the Q1–QN entries). These are the project's "load-bearing beliefs"; design-change findings may need to override one.
...
- **`D-NN` design change** — user wants a direction shift. The current behavior might be "working as specified" but the user wants different specs. Check for contradictions with `design-plan.md` Q-decisions before acting.
...
If this is a `D-NN` and the proposed change contradicts a Q-decision in `design-plan.md`, STOP — do not plan code changes yet. Q-decisions are load-bearing and can't be overridden silently.

Draft a new Q (use the next unused number, e.g. Q23) in `design-plan.md` after the last existing Q. The opening sentence must include the exact phrase **"supersedes QN's claim that ..."** — Q20/Q21/Q22 already used this pattern to override Q18. Show the draft Q to the user verbatim. Wait for their approval before writing any code. If they don't like it, adjust the draft; if they withdraw the change, mark the backlog row `rejected` with a reason.
...
- **`/check`** — used during verification. If the finding is Rust-only, `/check rust` is fine; otherwise `/check fast` or `/check all`.
