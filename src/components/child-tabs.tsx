      ...
      {items.map((child) => {
        ...
        return (
          <button
            key={child.id}
            type="button"
            className={`flex items-center gap-1.5 rounded-md px-4 py-1.5 text-[13px] font-medium transition-all duration-200 ${
              isSelected
          ...
          >
            {needsAttention && <span className="h-1.5 w-1.5 rounded-full bg-attention" />}
            {child.displayName}
          </button>
        );
      })}
